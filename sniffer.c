...
//	printf("Successful.");
	...
	//inet_ntoa(addr) = source.sin_addr.s_addr;
...
//	connect(sniffer, (struct sockaddr*)&dest, sizeof(dest));
//	bind(sniffer, (struct sockaddr*)&dest, sizeof(struct sockaddr));
	while (1) {
		if (bind(sniffer, (struct sockaddr*)&dest, sizeof(struct sockaddr)) < 0)
		{
			printf("Error : %d.\n", WSAGetLastError());
	...
	while (1) {
		if (WSAIoctl(sniffer, SIO_RCVALL, &j, sizeof(j), 0, 0, (LPDWORD)&in, 0, 0) != 0)
		{
...
void StartSniffing(SOCKET sniffer)
{
	...
	do
	{
		...
		if(mangobyte > 0)
		{
			ProcessPacket(sniffer,Buffer, mangobyte);
		}
		else
		{
